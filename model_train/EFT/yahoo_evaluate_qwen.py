...
# -*- coding: utf-8 -*-
"""
Concise evaluation script for Qwen (Yahoo topic classification).

Only the model loading is adapted for Qwen2.5-7B-Instruct;
all other logic and parameters stay the same as the original script.
"""
...
class SimpleEvaluatorQwen:
    ...
    def load_model(self):
        """Load the Qwen base model and LoRA adapter."""
        ...
        # Load tokenizer and model (Qwen requires trust_remote_code=True)
    ...
    def generate_responses(self, prompts: List[str]) -> List[str]:
        ...
        for prompt in prompts:
            ...
            # Generation parameters (kept consistent with the original script)
            with torch.no_grad():
    ...
    def extract_json(self, response: str) -> str:
        ...
        If a complete JSON object cannot be found, return the original response.
        """
        ...
        if brace_count == 0:
            ...
            # Incomplete JSON, return original response
            return response
    
    def evaluate_dataset(self, test_data: List[Dict], batch_size: int = 1) -> List[Dict]:
        """Evaluate the entire Yahoo test set with optional batching.

        The Yahoo test data format follows the original script
        ...
        for batch_start in range(0, total_samples, batch_size):
            ...
            for i, (sample, raw_response) in enumerate(zip(batch_samples, raw_responses)):
                ...
                # Validate whether the predicted label is a valid Yahoo topic
                try:
...
def main():
    parser = argparse.ArgumentParser(description="Simple Qwen Evaluation (Yahoo)")
