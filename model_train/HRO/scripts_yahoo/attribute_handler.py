...
def extract_attributes_from_input(input_text):
    ...
    for pattern in target_patterns:
        ...
        if target_match:
            label = target_match.group(1).strip()
            # 标准化Yahoo标签
            yahoo_labels = [
            ...
            # 检查是否匹配已知标签
            for yahoo_label in yahoo_labels:
                if yahoo_label.lower() in label.lower():
                    attributes['target_label'] = yahoo_label
                    break
            if 'target_label' not in attributes:
                attributes['target_label'] = label
            break
    ...
    # 从提示词中提取属性约束
    yahoo_attributes = [
        'question_type', 'user_intent', 'answer_tone', 'complexity_level',
        'evidence_expectation', 'style', 'domain_subtopic'
    ]
    
    for attr in yahoo_attributes:
        pattern = rf"- {attr.replace('_', ' ')}: ([^\\n]+)"
