...
# 添加脚本路径
# sys.path.append('/public/home/huzhenlin2023/paper_2_LLM_Synthesis/synthesis_model_train/TRL-GRPO-ohter-dataset/scripts_9_18')
...
MERGED_MODEL_PATH = "/public/home/huzhenlin2023/paper_2_LLM_Synthesis/synthesis_model_train/TRL-GRPO-ohter-dataset/amazon/merged_sft_grpo_sft_amazon_model"  # Amazon数据集专用模型路径
OUTPUT_DIR = "/public/home/huzhenlin2023/paper_2_LLM_Synthesis/synthesis_model_train/TRL-GRPO-ohter-dataset/amazon/synthesis_grpo_reward_amazon_sft_grpo_enhanced_output-sim500-epo2-bat4-gr4-gen4"
...
ENHANCED_CONFIG = {
    ...
    "num_train_epochs": 2.0,        # 稍微增加训练轮数
    "per_device_train_batch_size": 4,
    "gradient_accumulation_steps": 4,
    "num_generations": 4,           # 增加生成候选数，提供更多选择
    "max_completion_length": 800,
    "logging_steps": 5,             
    "save_steps": 40,
    "learning_rate": 3e-6,          # 稍微提高学习率
...
# 🎯 重新调整的奖励权重配置
# 基于production版本表现，调整权重分配
ENHANCED_SAMPLE_REWARDS_CONFIG = {
    "sentiment_consistency_weight": 0.1,    # 情感权重（最重要）
    "attribute_compliance_weight": 0.1,     # 属性权重（重要）
    "generation_quality_weight": 0.5,       # 生成质量权重（包含长度和JSON格式）
    # 移除单独的长度奖励，融入到生成质量中
}
...
GENERATION_CONFIG = {
    ...
    "top_p": 0.92,                 # 稍微放宽
    "top_k": 60,                   # 增加词汇选择
    "repetition_penalty": 1.12,     # 稍微降低重复惩罚
    "pad_token_id": None,
}
...
def create_enhanced_reward_functions():
    """创建增强版奖励函数，面向Amazon提示格式提取真实属性并计算奖励"""
...
def load_and_process_data(data_file, max_samples=None):
    ...
    for item in data:
        ...
        # 方法3：兜底逻辑：按优先级搜索（确保very positive在positive之前）
        if target_sentiment == 'neutral':
    ...
    try:
        ...
        for log_entry in log_history:
            if 'step' in log_entry:
                ...
                # length_rewards.append(log_entry.get('rewards/reward_length_compliance/mean', 0))  # 已移除长度奖励
...
def parse_args(argv=None):
    """解析命令行参数，兼容pipeline传入的额外位置参数 (data_file, base_model, output_dir ...)"""
    parser = argparse.ArgumentParser(description="Amazon GRPO reward enhanced training", add_help=True)
    ...
    parser.add_argument("--dry-run", action="store_true", help="仅初始化各组件，不执行trainer.train()，用于快速验证")
    parser.add_argument("--override-max-train-samples", type=int, default=None, help="覆盖配置中的max_train_samples以做更小规模试跑")
    # 捕获未知参数（pipeline会把脚本路径和其他位置参数传进来）
...
def main():
    """主训练流程"""
    # 使用当前 sys.argv 进行解析（忽略pipeline注入的多余位置参数）
    args = parse_args(sys.argv[1:])
    print("🌟 启动Amazon数据集奖励增强版GRPO训练...")
    print("🎯 核心策略: 适配Amazon评论生成，放宽评分标准，提高奖励上限，促进学习")
    ...
    print("⚙️  属性约束: 13个Amazon特有属性（产品类别、使用情境、评论角度等）")
    
    print("🎯 奖励增强策略:")
    print(f"   情感一致性: 上限1.0 (原[0, 0.5] → [0, 1.0])")
    print(f"   属性符合度: 上限1.0 (有关键词就给高分)")
    print(f"   生成质量: 上限1.0 (包含长度和JSON格式检查)")
    ...
    # 初始化Amazon数据集奖励函数模块的全局变量
    initialize_reward_globals(
        training_data_global, 
        ENHANCED_CONFIG['per_device_train_batch_size'],
        reward_calculator, 
        novelsum_calculator,
        attr_loader,  # 这里是amazon_config
    ...
    if args.enable_dynamic_reward:
        ...
        try:
            ...
            print("🔄 已启用动态连续奖励系统")
            print(json.dumps(dynamic_config_summary, ensure_ascii=False, indent=2))
        except Exception as e:
    ...
    try:
        ...
        summary_path = os.path.join(OUTPUT_DIR, "enhanced_run_config_summary.json")
        summary_payload = {
        ...
        print(f"📝 运行配置概要已保存: {summary_path}")
    except Exception as e:
        print(f"⚠️ 配置概要保存失败: {e}")
    
    if args.dry_run:
        print("🚫 Dry-run模式: 跳过trainer.train()，只做初始化验证。")
        return
    ...
    print("\n" + "=" * 80)
    print("🎉 Amazon数据集奖励增强GRPO训练完成！")
    print(f"   训练时长: {training_time:.2f}分钟")
    print(f"   处理样本: {ENHANCED_CONFIG['max_train_samples']}")
    print(f"   输出目录: {OUTPUT_DIR}")
    print("   Amazon专用奖励特性:")
    print("     - ✅ 情感一致性：基于五类情感等级精确匹配")
    print("     - ✅ 属性符合度：13个Amazon属性约束检查")
    print("     - ✅ 生成质量：Amazon评论特点和语言风格评估")
