...
Used to generate line plots and tables for rewards and losses.
"""
import os
import json
import pandas as pd
...
# Configure fonts and plot style; fall back to English-only fonts if needed
try:
...
class TrainingVisualizer:
    """Visualizer for the GRPO training process."""
    ...
    def record_reward_data(self, step: int, reward_type: str, rewards, completions):
        """Record detailed statistics for reward functions."""
        import torch
        
        # Ensure the step exists in the records
        if step not in self.training_metrics['steps']:
            self.training_metrics['steps'].append(step)
            # Fill default values for other metrics
        ...
        # Record values by reward type
        if reward_type in self.training_metrics['sample_rewards']:
            # For sample-level rewards, update current step if it exists, otherwise append
    ...
    def create_reward_plots(self):
        """Create line plots for reward trends."""
        ...
        print(f"📊 Reward trend plot saved to: {plot_path}")
        plt.close()
    
    def create_summary_table(self):
        """Create a summary table for training metrics."""
        if not self.training_metrics['steps']:
            print("⚠️ No training data, skip table generation")
            return
        ...
        if total_steps > 0:
            ...
            summary_data.append({
                'metric_type': 'Training overview',
                'metric_name': 'Total training steps',
                'final_value': total_steps,
                'mean_value': total_steps,
                'max_value': total_steps,
                'min_value': total_steps
            })
            
            summary_data.append({
                'metric_type': 'Overall reward',
                'metric_name': 'Total reward',
                'final_value': f"{final_total_reward:.4f}",
                'mean_value': f"{avg_total_reward:.4f}",
    ...
    def create_detailed_metrics_plot(self):
        ...
        fig.suptitle('📈 GRPO Detailed Metrics Analysis', fontsize=16, fontweight='bold')
        
        steps = self.training_metrics['steps']
        
        # 1. All sample-level rewards
        ax1 = axes[0]
        colors = ['#FF6B6B', '#4ECDC4', '#45B7D1']
        for i, (reward_type, values) in enumerate(self.training_metrics['sample_rewards'].items()):
            if values:
        ...
        ax1.set_title('Sample-level Reward Comparison', fontsize=14, fontweight='bold')
        ax1.set_xlabel('Training step')
        ax1.set_ylabel('Reward')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # 2. All batch-level rewards
        ax2 = axes[1]
        colors = ['#96CEB4', '#FFEAA7']
        for i, (reward_type, values) in enumerate(self.training_metrics['batch_rewards'].items()):
            if values:
        ...
        ax2.set_title('Batch-level Reward Comparison', fontsize=14, fontweight='bold')
        ax2.set_xlabel('Training step')
        ax2.set_ylabel('Reward')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        # 3. Total reward vs loss (dual Y-axis)
        ax3 = axes[2]
        ax3_twin = ax3.twinx()
        ...
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        print(f"📊 Detailed metrics analysis plot saved to: {plot_path}")
        plt.close()
    
    def save_raw_data(self):
        """Save raw training metrics to disk."""
        ...
        print(f"💾 Raw training metrics saved to: {raw_data_path}")
    ...
    def generate_final_report(self):
        ...
        print("\n📊 Training visualization report generated!")
        ...
        print("   - training_summary.csv: training summary table")
        print("   - training_summary.html: HTML summary report")
        print("   - raw_training_metrics.json: raw metrics data")
        
        # Print a short textual summary
        if summary_df is not None and not summary_df.empty:
                print("\n📈 Training summary:")
                total_reward_row = summary_df[summary_df['metric_name'] == 'Total reward']
            if not total_reward_row.empty:
                print(f"   Final total reward: {total_reward_row.iloc[0]['final_value']}")
                print(f"   Mean total reward: {total_reward_row.iloc[0]['mean_value']}")
...
def log_training_step(step: int, metrics: Dict[str, Any]):
    """Proxy to log metrics for a single training step."""
    global training_visualizer
    if training_visualizer:
        training_visualizer.log_step_metrics(step, metrics)

def generate_final_training_report():
    """Proxy to generate the final training report using the global visualizer."""
    global training_visualizer
    if training_visualizer:
